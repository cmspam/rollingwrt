...
sign_feed() { # $1=buildroot(for apk+keys) $2=dir-of-apks -> build+sign packages.adb
	...
	# --allow-untrusted: index our OWN freshly-built apks without re-verifying each apk's
	# signature against the keystore first. apk-tools 3 keys a signature by the signer's
	# identity, and a bare public-key.pem in --keys-dir does not match it, so mkndx would
	# reject the very apks it is about to index (seen when the collected packages come from
	# a different job than the one indexing them). It is unneeded here: the SIGNED index is
	# the trust anchor - the device verifies packages.adb with our key, then trusts each
	# package by the checksum the index vouches for, never the loose per-apk signature.
	( cd "$2" && ls ./*.apk >/dev/null 2>&1 && "$apk" mkndx --allow-untrusted --keys-dir "$1" "${sign[@]}" -o packages.adb ./*.apk )
}
...
prepare_tree() { # $1=tree dir $2=pin
	...
	# KVM_IOAPIC: in-kernel irqchip for the hypervisor (OpenWrt disables it). Goes in the
	# parent x86 config; its dependency (KVM) is already met there. Neither this nor the
	# EFIVAR_FS below changes the kmod vermagic.
	for kc in "$owrt"/target/linux/x86/config-*; do
		grep -q '^CONFIG_KVM_IOAPIC=y' "$kc" || echo 'CONFIG_KVM_IOAPIC=y' >> "$kc"
	done
	# MODULE_STRIPPED: OpenWrt strips version/srcversion/param-description modinfo from every
	# kmod to save flash. incus reads the zfs module version (/sys/module/zfs/version and
	# `modinfo -F version zfs`) and refuses the ZFS storage driver when it is absent, so we
	# keep it - flash size is irrelevant on a disk system. It is =y in the GENERIC config,
	# and a parent/subtarget override does not reliably flip a generic =y, so disable it at
	# the source.
	for gc in "$owrt"/target/linux/generic/config-*; do
		sed -i 's/^CONFIG_MODULE_STRIPPED=y/# CONFIG_MODULE_STRIPPED is not set/' "$gc"
	done
	# EFIVAR_FS: efivarfs, so a running UEFI system can read/write the firmware's
	# SecureBoot/SetupMode variables (needed for on-device rollingwrt-secureboot
	# status/enroll). It `depends on EFI`, and generic/config disables EFI while only the
	# x86/64 SUBTARGET config re-enables it - so EFIVAR_FS must go in that same subtarget
	# config, or olddefconfig drops it (its dep is unmet when the parent config is read).
	for kc in "$owrt"/target/linux/x86/*/config-*; do
		case "$kc" in *"/x86/64/"*|*"/x86/generic/"*)
			grep -q '^CONFIG_EFIVAR_FS=y' "$kc" || echo 'CONFIG_EFIVAR_FS=y' >> "$kc" ;;
		esac
	done
...
if [ "$STAGE" = index ]; then
	...
	if [ -n "${SDK_IN:-}" ]; then
		...
		( cd "$DEST" && apk mkndx --allow-untrusted -o packages.adb ./*.apk )
	fi
