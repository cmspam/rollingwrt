...
# Stable name: the installer pulls the system from the feed at install time, so a
# rebuilt ISO supersedes the old one - keep a single "latest" asset, not dated copies.
ISO_NAME="rollingwrt-installer-latest"
...
PKGS=(
	...
	# dialog drives the installer's ncurses menu interface
	dialog
...
# Our feed's public key. Derive it straight from the signing secret when the build has
# it (CI does): that way the ISO always carries the key, even on the very first build
# after the feed releases were wiped, when the published public-key.pem does not exist
# yet. Fall back to the released copy for a keyless local build. An ISO with no key
# cannot verify our packages, so treat a total miss as a build failure, not a warning.
if [[ -n "${APK_SIGNING_KEY:-}" ]] && \
   printf '%s' "$APK_SIGNING_KEY" | openssl pkey -pubout -out "$ROOTFS/etc/apk/keys/rollingwrt.pem" 2>/dev/null; then
	echo "    derived our feed key from the signing secret"
elif curl -sfL "$RWRT_PUBKEY_URL" -o "$ROOTFS/etc/apk/keys/rollingwrt.pem"; then
	echo "    staged our feed key from $RWRT_PUBKEY_URL"
else
	echo "ERROR: no feed key (no APK_SIGNING_KEY secret and $RWRT_PUBKEY_URL unreachable)" >&2
	rm -f "$ROOTFS/etc/apk/keys/rollingwrt.pem"; exit 1
fi
