...
On install, rollingWRT builds a signed [Unified Kernel Image](./boot-and-security/) on
the machine and boots it with systemd-boot. The root filesystem is LUKS2-encrypted and
...
supported with per-machine keys and no shim or vendor key. See
[Boot and Security](./boot-and-security/).
